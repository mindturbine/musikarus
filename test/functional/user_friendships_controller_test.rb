    ...
    context "when logged in" do
      ...
      context "with no friend_id" do
        ...
        context "with a valid friend_id" do
          setup do
            post :create, user_friendship: { friend_id: users(:gabe) }
      end

        should "assign a friend object" do
          assert assigns(:friend)
          assert_equal users(:jonathan), assigns(:friend)
        end
        ...
        should "redirect to the profile page of the friend" do
          assert_response :redirect
          assert_redirected_to profile_path(users(:gabe))
        end

        should "set the flash success message" do
          assert flash[:success]
          assert_equal "You are now friends with #{users(:gabe).full_name}", flash[:success]
        end
      end
    end
  end
